...
// WriteHeader allows passing custom HTTP header. Once common use case is to pass `X-Scope-OrgID` header for Cortex tenant.
func WriteHeader(key, value string) WriteOption {
	return func(opt *writeOptions) {
		if opt.headers == nil {
...
func (p *Client) Write(ctx context.Context, req *WriteRequest, options ...WriteOption) (*WriteResponse, error) {
	...
	pbRaw, err := proto.Marshal(&prompb.WriteRequest{
	...
	pbCompressed := snappy.Encode(nil, pbRaw)

	// Prepare http request.
	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, p.endpoint, bytes.NewBuffer(pbCompressed))
	if err != nil {
		return nil, err
	}
